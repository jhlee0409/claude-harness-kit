...
| `agents/claim-checker.md` | A read-only critic that falsifies a terminal/analytical claim ("limit", "sufficient", "fully solves") — was it measured or asserted, and was the falsifier run? An independent check at the conclusion boundary (the marginal-but-real lever against overclaim; an automated hook can't do this — provenance words are forgeable). |
